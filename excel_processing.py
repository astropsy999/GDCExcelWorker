...
# def process_excel_file_oen(file_path, installation_name, control_date):
    ...
    if cell_L5.value:
        ...
        for row in range(2, sheet.max_row + 1):
            sheet.cell(row=row, column=2).value = f'=CONCATENATE(B{row}; "."; C{row}; ".")'
    else:
        # Копируем значения из столбца В в столбец А
        for row in range(2, sheet.max_row + 1):
            set_value_if_not_merged(sheet, row, 1, sheet.cell(row=row, column=2).value)
    ...
    set_value_if_not_merged(sheet, 7, 6, installation_name)

    # Проверка значения в ячейке на соответствие формату ХХ.ХХ.ХХХХ
    cell_value = sheet['F7'].value
    pattern = re.compile(r'^\d{2}\.\d{2}\.\d{4}$')
    if not pattern.match(cell_value):
        # Если значение не соответствует формату, пытаемся удалить "г." из даты
        new_value = cell_value.replace("г.", "")
        set_value_if_not_merged(sheet, 7, 6, new_value)
    ...
    if cell_L5.value:
        ...
        for row in range(2, sheet.max_row + 1):
            ...
            if isinstance(cell, MergedCell):
                ...
                merged_range = next((mr for mr in sheet.merged_cells.ranges if mr.min_row <= row <= mr.max_row and mr.min_col <= 2 <= mr.max_col), None)
                if merged_range:
